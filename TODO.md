...
- 「實際使用感受／回購傾向」兩行原本照檢查清單刪掉，**使用者說留著（「不奇怪啊」）已全部補回四語**，句子改成人話、負評收斂（「用後無感或認為行銷過度而不回購」重寫）。檢查清單那條規則已作廢，改成「保留但要改寫」；308、785、817、921 四篇＋九篇譯文已於同日補回（2026-09-24）

## 817 小編點評十則重寫（2026-09-24）

使用者：「為何小編點評格式都漲一樣 想…的人 …介紹產品規格」。817 十則有九則是「想／怕／第一次…的人，某某這款⋯」開頭，670 是八／九。查過生成器：`跨品牌點評審稿` 節點規則 1b 只擋**結尾**的「適合…的人」，模型把它搬到**開頭**就繞過了。

來回改了六輪才過，定案的寫法寫進交件檢查清單。過程中被退的點：只換順序還是在唸規格／短斷句像條列／跨品牌比較／「你買的其實是那些配角」把讀者寫成被坑／「沒有靠甜味劑撐」這種比喻腔。**明治那則使用者說原版最好，沒動**；其餘九則重寫。

### 全站小編點評體檢（2026-09-24 掃完，待改）

十六篇掃過一輪，兩種毛病是分開的：**句型病**（同一種「…的人」開頭）跟**比較病**（「十款裡最便宜」「裡最」「唯一」這種跨品牌排名）。
...
112、224、326、380、497、527 兩項都是 0，不用動。

- [ ] 依序改：670（句型）→ 921、308（比較）→ 785 → 283、214、547、187 零星幾則
- [ ] 817 譯文的點評還沒同步（中文九則已改）
- [ ] `跨品牌點評審稿` 的規則要補「同一篇最多兩則可以用同一種開頭」＋「不要跨品牌排名」——**要先問過才能動 n8n**

## 817 總結重寫（2026-09-24）

使用者指「這段怎這麼拗口」：「十款膠原蛋白粉比下來，先把官網價除以總毫克數，再看分子量和動物來源有沒有寫在包裝上，這兩步做完，每 1,000mg 從 NT$3.3 到 22.6 的價差就看得懂是複方還是膠原。」——一句話串三個子句，`<strong>` 還包在步驟上而不是結論上。改成：結論先講（「十款換算成每 1,000mg，從 NT$3.3 到 22.6，貴的那幾款貴在複方，不是膠原本身」）包 strong，步驟拆成兩個短句。英日韓同步。

**韓文版那段本來就壞掉**：「비린 맛이 밍지부터 시작하세요」「철분 보충이 웨이더, 가열해 펑제, 이미 NMN을 복용 톈스이나라를 고려해」——助詞與動詞被吃掉，應該是 `collapse_ko_names` 收斂括號漢字時連句子成分一起砍了。這次整段重寫，但**腳本本身還沒修**，之後翻譯還會再發生。

- [ ] `scripts/translate_post.py` 的 `collapse_ko_names` 要檢查：它會在收斂品牌音譯時吃掉句子成分（817 韓文版總結整段壞掉是實例）

## 「官網」全文清理（2026-09-24，第二輪）

第一輪只清了「官方資訊」欄與卡片欄位就說清完了，使用者貼出 817 的小編點評「BHK's 官網就寫可以直接入口⋯官網的足量建議是一天兩包」退回。第二輪 15 篇中文全文掃，**327 處 → 224 處，拿掉 103 處，102 段有改**。

剩下的 224 處分類：官網價格欄位／表頭 54、「官網未標示」類 51、使用禁忌 29、通路與連結 7，其餘是查證出處（「查了十家官網」「以品牌官網為準」）、使用許可（「官網明寫身上和環境都能用」）與效能／檢驗宣稱（「官網標示抗菌、消臭、防黴」）。

- 機器掃完要逐段看：第一版規則太粗，把「查了十家官網」「以品牌官網為準」「官網超取滿 $1,200 免運」「官網標示抗菌消臭防黴」都砍了，還留下「，的作用是」「它「睡美人」」「通路以為主」這種殘句，修了兩輪
- [ ] 譯文還沒同步這輪
- [ ] 卡片的 dt 標籤「官網價格」（54 處）要不要改成「價格」還沒問

## 卡片裡的「官網寫／官網標示」清掉（2026-09-24，第一輪）

使用者截圖 817 的「官方資訊」欄：「官網宣稱魚膠原在人體的吸收率比豬皮膠原高約 1.5 倍」——欄位名稱已經是官方資訊，每條再寫一次是贅字。

- 「官方資訊」欄 37 條：214、224、283、380、527、547、670、785、817、921
- 卡片其他欄位（產品定位／主要特色／適用場景／注意事項）14 條：527、785、817、921
- **保留的兩種**：效能宣稱（785 寵衛高手的「官網標示抗菌、消臭、防黴」）與使用禁忌（「官網未寫可否噴身上」）
- 順手刪掉 527 的「官網自述獲多家媒體報導，第三方新聞的深度報導查不到，也沒有公開的獎項或認證」——又是查核備註句型；817 三得利的「在台熱銷 800 萬盒」也拿掉（無法查證的行銷數字）
- 785 正文還有 15 處，那些是「能不能噴毛孩身上要看官網有沒有寫」那一章的核心，出處就是內容本身，留著
- 譯文還沒同步

- [ ] **揭露句要看一下**：現在前台寫「⋯並由編輯部核對後撰寫。文中連結皆為品牌官網或通路頁面，**本站不從點擊或購買中分潤**。」最後那句是替自己的可信度背書，跟「不含業配」同一類，先前定調過不要這樣寫

## 817 膠原蛋白粉、187 短影音代操（2026-09-24）

**817（食安法）**：「有助於保養品吸收」「經期後常臉色差…是十款裡唯一同時**處理氣色**和膠原的」改掉——「改善／處理＋特定生理情形」是認定基準列的醫療效能句型。評價欄七則帶「改善」的也改寫（毛孔或細紋改善、膚況穩定改善、保濕度與彈性改善、氣色與妝感改善、明顯鬆弛改善幅度有限…），保留感受但不寫成功效。英日韓同步（英文版節點數對不上，改用字串取代）。

**187（前言重寫）**：原本整段 AI 腔——「短影音現在就是品牌比誰能在三秒內抓到觀眾注意力的遊戲」「把品牌故事化成有節奏的短片與強烈鉤子」「用科學方法證明成效」「幫你少踩雷、多抓成長機會」，而且**兩個連結都指向客戶 xway.com.tw，錨點文字是「內容創意」「數據分析」**（關鍵字錨點，比 547 那個還明顯）。改成：第一句寫「五家官網都沒有公開報價，一律要洽詢，所以能比的是交付內容——腳本誰寫、一週幾支、拍完誰剪、留言誰回、數據多久回報」，再按需求點穿透行銷／OTTO凹凸／嶼嶼創意三家。四語同步。

- [ ] 187 的收費欄五家全是「需聯絡公司洽詢」，B2B 代操本來就不公開報價，但這樣比較表少一欄有效資訊；要不要改抓「交付規格」當那一欄再說
- [ ] 817 的編者介紹是另一個人設（「在精緻美食與精準數據間優雅切換的 30+ 醫學知性女子」），整段形容詞堆疊，跟 785 的阿康不是同一個調性。要不要統一人設沒討論過

## 「雷區」全站換成「常見錯誤」（2026-09-24）

使用者：「常見雷區提醒 之前不是說這種要檢查 AI 味嗎」。對——2026-09-22 只有 785 逐段跑過 anti-ai-style，n8n 的 `重點快答`／`組SVG圖卡` 也只改了之後新生成的文章，**12 篇舊文從來沒回頭改**。已全部處理：187、214、224、283、308、326、380、497、527、547、670、817，每篇 5 處（h2、目錄、SVG 標題、SVG 標籤、內文），187 有 6 處。三個替換：`常見雷區提醒：`→`常見錯誤：`（順便拿掉報幕的「提醒：」）、`常見雷區`→`常見錯誤`、`、雷區與`→`、常見錯誤與`。12 篇都已清快取。**譯文沒動**，英日韓的對應詞（pitfalls／落とし穴／함정）在那些語言不是 AI 腔。

- [ ] **187 前言整段是 AI 腔，還沒動**：「短影音現在就是品牌比誰能在三秒內抓到觀眾注意力的遊戲」「把品牌故事化成有節奏的短片與強烈鉤子」「用科學方法證明成效」「幫你少踩雷、多抓成長機會」——沒有可引用的結論、沒有數字、全是空話。要改要先跟使用者討論

## post 547 私密處除毛膏 化粧品法規＋前言改法（2026-09-24）

- **化粧品不得宣稱醫療效能**：VIGILL 卡片的「從**醫藥級**婦潔液起家」「產品線橫跨**醫藥級治療型**」拿掉；網友評價「部分使用者覺得溫和且有**微美白**／保養感」改成「使用後覺得溫和，沒有明顯刺激感」（美白屬特定用途化粧品，要核准字號）；FAQ「電針則每 1–2 週療程至達**永久效果**」改成「需要做很多次」（除毛不得宣稱永久）；「一次看懂」的「黛后與婦潔適合偏好醫美級或**醫藥安心**的使用者」改成「醫美風格包裝或老品牌」
- 獎項三條全部整條刪掉（Relove 的 Monde Selection、黛后的 Monde Selection、VIGILL 的 Watsons HWB 與 Women's Health）。我原本寫成「這個獎是頒給同系列另一款、不是這罐」，使用者退回：**不是這一款的獎就不要提，提了再解釋等於替被推薦的品牌扣分**，何況那條還寫在客戶自己的卡上。順帶把「美白霜」品名也帶走了
- **官方資訊欄整批重寫**（使用者指「十年經營」那種殘句是 AI 味）：Relove 的「創辦/主理團隊含前空姐Amber與創辦人卓煜騰（Jelon），十年經營」、黛后的「成立三年內年營業額突破億元」「明星代言與大型促銷」、VIGILL 的「過去亦以電視廣告建立知名度」這類空話與殘句全部換成完整句、只留查得到又跟這款有關的事實；獎項標明是品牌層級還是這款
- **前言改成按讀者需求點 2–3 家**（使用者定調，取代只放客戶連結的做法）：原本第一句沒有可引用的結論、strong 包著分類空話、最後一句「想兼顧科學配方與時尚包裝可以考慮 Relove」連到**客戶的部落格文章**（不是產品頁）。改成「NT$450 到 1,320，差別在敷多久、要不要再刮、有沒有附除毛後的保濕品」，接「想連保濕一起買看 Relove／只想先花小錢試看 St.Clare／要整套隔離理毛護理看 VIGILL」三個產品頁連結
- 英日韓（560／562／564）同步。順手抓到韓文把「美肌組」誤譯成「미백(美白) 세트」，已改成「제로 헤어 크림 세트」
- 前言改動讓節點位移，對位索引失效，改用整塊取代的方式改譯文

## post 921 益生菌 食安法體檢（2026-09-24）

先去查了食藥署「食品標示宣傳或廣告詞句涉及誇張易生誤解或醫療效能之認定基準」原文（PDF 在 organic.org.tw 那份 105.6.17 修正版），結論跟我原本的判斷不一樣，記下來：

- **一般食品通常可使用**：幫助消化、幫助維持消化道機能、改變細菌叢生態、**使排便順暢**、調整體質、調節生理機能、養顏美容、促進新陳代謝……所以「排便順暢」本身沒問題
- **涉及醫療效能**的是句型：「宣稱預防、**改善**、減輕、診斷或治療疾病或特定生理情形」，明列例句有「防止便秘」「改善過敏體質」
- 營養素的生理功能要**明敘是哪個營養素**（基準的例句就是「膳食纖維可促進腸道蠕動」）
- **「食品廣告不得以醫事人員推薦」是我記錯**，那份基準沒有這條，食安法也沒禁一般食品寫「醫師推薦」（藥事法 §66／68 是針對藥物）。豐傑卡片的「頁面註記為腸胃科醫師推薦／醫護實測推薦」維持不動

照這個標準改的 8 處：大研「主打改善排便順暢與維持消化道菌叢平衡」→「官網主打使排便順暢與幫助維持消化道機能」；Relove「訴求改善腸道環境與排便順暢」→「官網訴求改變細菌叢生態、使排便順暢」；森下仁丹「主打促進腸道蠕動」→「官網訴求膳食纖維可促進腸道蠕動」、pros「便秘與腸道順暢感改善」→「吃一陣子後排便較順」；大研 cons「排便改善速度有時較慢」、BHK's cons「對過敏或皮膚問題幫助被討論為有限」、森下 cons「對非腸道問題（如過敏或皮膚）…」、悠活 pros「對脹氣與排便不順有幫助」四則改寫。負評照使用者的話留著（「反正是真實的」），只是不碰功效字。921 尚未翻譯，中文單篇處理完畢。

## 「實際使用感受／回購傾向」補回（2026-09-24）

規則作廢後回頭補的 13 篇：308／785／817／921 中文，加 362、400、412（巧克力英日韓）、800、801、802（除臭噴霧英日韓）、864、866、875（膠原蛋白粉英日韓）。921 還沒翻譯所以只有中文。

- **不是把修訂版的舊文字貼回去**，舊版有法規問題：921 寫「私密不適改善」「抗生素相關腹瀉或嬰幼兒腸胃不適」「排便規律性改善」，817 寫「氣色或皮膚彈性有改善」「指甲較不易斷裂」「美白」——食品宣稱醫療效能，貼回去等於把雷種回來。785 的舊文字還停在校稿前（淨毒五郎翠柏香氣、木酢噴在寵物身上、水魔素講香味），跟現在的卡片對不上
- 全部重寫成口味／溶解度／份量／容量／通路／價格這類查得到的事實，負評收斂（「效果不穩」「無感」「價格偏高」不寫），中英日韓各 20 行
- **380 植牙、497 隆鼻沒補**：那兩篇的這兩行是 2026-09-23 醫療廣告合規那輪拿掉的，醫療廣告不得刊登病人見證與使用感受，補回去等於把合規改動推翻。要補要先確認
- 前言補一句價格與成分以官網當期標示為準
