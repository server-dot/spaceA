...
- [x] 生成器接客戶 Drive 資料（2026-09-22，n8n `推薦文-3-完整生成` 新增 14 個節點，已驗證能讀到）：`抓人設` 之後先走 `找客戶資料夾 → 挑客戶資料夾 → 列客戶子資料夾 → 挑素材子資料夾 → 列客戶檔案 → 展開客戶檔案 → 是PDF？→（下載＋抽 PDF 文字｜匯出 Docs／Sheets 文字）→ 合併 → 整理客戶資料`，再接原本的五個下游。用 Drive 憑證 `Google Drive account`（server@stack.com.tw）在「客戶」資料夾（`1NS72zWdfXdMxYlS4YliNa7P4DVQUpfhU`）找名字含必含品牌的子資料夾（多個候選時挑跟標題重疊字最多的，貝恩嬰兒／貝恩寵物就是這樣分），只讀「文章素材」「產品資訊」兩個子資料夾，跳過會議記錄和檔名含會議／合約／報價／進度的檔（合約費用不能進文章），最多 12 檔、總量 16000 字元
  - 餵給誰：`前三章`（三大重點）、`比較段落`、`faq細項`、`重點快答` 拿到整份當「品類資料」，規則寫死只能用品類層級事實、不點名品牌、不因此偏袒；`官方資訊`、`轉化html卡片` 只有品牌名含必含品牌那一家拿得到（補全成分、認證、用法、保存期限），並明講點評跟別家一樣長、卡不能比別家厚
  - 新增 `客戶問題補FAQ`（LLM）→ `客戶FAQ拆項` → `合併FAQ` → `收合容器樣式`：從飼主需求文件挑最多 3 題大綱沒問到的品類問題接在 FAQ 後面，不點名品牌。沒客戶資料時整條照跑、輸出空
  - 內部備註（「不要強調、可食用酒精」）：prompt 寫了「客戶說不要強調的事不寫」，但校稿還是要看
  - **下一篇推薦文生成完要驗證**：`整理客戶資料` 有沒有抓到（client_files）、FAQ 是否多出 0～3 題、客戶那張卡有沒有變厚、三大重點有沒有偷偷點名客戶
  - 客戶端要做的：「貝恩寵物 產品飼主需求」這類「存取權受限」的文件要分享給 server@stack.com.tw 才讀得到（貝恩那份已可讀）
...
- [ ] 寫新文章 — 十篇交辦只剩寵物用品（2026-09-21 核對；下面清單是進度）
  ...
  - [x] 寵物用品 — post 785 `/pets/best-pet-deodorizing-sprays/`（客戶貝恩，題目定「寵物除臭噴霧」；10 家：貝恩、臭味滾、寵衛高手、汪喵星球、Aether 依鈦、毛天使、淨毒五郎、梧天家、水魔素、木酢達人；2026-09-22 生成＋校稿完成，見下方）
